...
class FANExtractor(object):
    ...
    def crop(self, image, center, scale, resolution=256.0):
        ...
        if image.ndim > 2:
            newDim = np.array([br[1] - ul[1], br[0] - ul[0], image.shape[2]], dtype=int32)
        ...
        ht = image.shape[0]
        wd = image.shape[1]
        newX = np.array([max(1, -ul[0] + 1), min(br[0], wd) - ul[0]], dtype=int32)
        newY = np.array([max(1, -ul[1] + 1), min(br[1], ht) - ul[1]], dtype=int32)
        oldX = np.array([max(1, ul[0] + 1), min(br[0], wd)], dtype=int32)
        oldY = np.array([max(1, ul[1] + 1), min(br[1], ht)], dtype=int32)
